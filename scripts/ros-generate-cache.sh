...
#
# Usage: cd meta-ros
#        sh scripts/ros-generate-cache.sh ROS_DISTRO YYYYMMDD PATH-TO-LOCAL-ROS-ROSDISTRO ROS-ROSDISTRO-COMMIT
#            or
#        sh scripts/ros-generate-cache.sh --version
#
#        Generate and commit a files/ROS_DISTRO/cache.yaml for the YYYYMMDD release of ROS_DISTRO from the specified commit of a
#        local ros/rosdistro.git . YYYYMMDD is taken from the release announcement or the last field of the
#        "release-ROS_DISTRO-YYYYMMDD" tag; if running prior to first release of ROS_DISTRO, specify "pre-release" for YYYYMMDD.
#        NB. The release date generally does not match the commit timestamp of ROS-ROSDISTRO-COMMIT; however, the commit timestamp
#        is used for the DATETIME when forming the name of the created branch.
#
#        Also, copy the files from PATH-TO-LOCAL-ROS-ROSDISTRO/rosdep to files/ROS_DISTRO/rosdep. The script will abort if the
#        entires in /etc/ros/rosdep/sources.list.d/20-default.list do not point to files under PATH-TO-LOCAL-ROS-ROSDISTRO/rosdep.
#
...
usage() {
    ...
    echo "Usage: sh scripts/$SCRIPT_NAME.sh ROS_DISTRO YYYYMMDD PATH-TO-LOCAL-ROS-ROSDISTRO ROS-ROSDISTRO-COMMIT"
    echo "               or"
